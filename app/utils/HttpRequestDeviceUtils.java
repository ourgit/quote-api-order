...
public class HttpRequestDeviceUtils {
    ...
    public static final int TYPE_MINIAPP = 4;
    ...
    private static String[] appHeaders = new String[]{
            "ios_app",
            "android_app",
    };
    private static String[] wxHeaders = new String[]{
            "micromessenger",
    };
    /**
     ...
     */
    public int getMobileDeviceType(Http.Request request) {
        int flag = 0;
        Optional<String> via = request.getHeaders().get("Via");

        if (via.isPresent()) {
            for (int i = 0; i < mobileGateWayHeaders.length; i++) {
                if (via.orElse("").contains(mobileGateWayHeaders[i])) {
                    flag = TYPE_MOBILE;
                    break;
                }
            }
        ...
        if (flag == 0) {
            ...
            if (userAgent.isPresent()) {
                for (int i = 0; i < wxHeaders.length; i++) {
                    if (userAgent.orElse("").toLowerCase().contains(wxHeaders[i])) {
                        flag = TYPE_MINIAPP;
                        break;
                    }
                }
                for (int i = 0; i < appHeaders.length; i++) {
                    if (userAgent.orElse("").contains(appHeaders[i])) {
                        flag = TYPE_APP;
                        break;
                    }
                }
                if (flag == 0) {
                    for (int i = 0; i < mobileUserAgents.length; i++) {
                        if (userAgent.orElse("").contains(mobileUserAgents[i])) {
                            flag = TYPE_MOBILE;
                            break;
                        }
                    }
                    if (flag == 0) {
                        for (int i = 0; i < pcHeaders.length; i++) {
                            if (userAgent.orElse("").contains(pcHeaders[i])) {
                                flag = TYPE_PC;
                                break;
                            }
                        }
